% ===================================================================
% This is the main function of the UAV deterrence project with the final
% (hybridized) strategy
%
...
target             = Target(false, 800, 700,  20, deg2rad(270), 5,  350,  12,      deg2rad(45));
targets             = [target];
...
% set k parameter and dive parameters
k = 0.6;
inter_mult_dive = 2;
offset_mult = 0.4;
closing_dist = k*FID;
...
[b, curve_length] = bezierPath(world, agents(1), targets(1), edge, q, plotting);
[b_dive, curve_length_dive] = bezierPathdive(world, agents(1), targets(1), edge, inter_mult_dive, offset_mult, plotting);
...
while t <= t_total && outofBounds == 0
    ...
    for i = 1:size(agents,1)
        ...
        % agents(i)   = moveAgentC(agents(i), world, cumulation, dt, targets(1));
