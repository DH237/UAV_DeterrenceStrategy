...
%             end
        ...
        function [obj] = updateInterestsDive(obj, agents, world, cumulation)
            ...
            for i = 1:size(agents,1)
                ...
                lambda1 = world.inter_alert_radius*1.3;      % arbitrary 1.5 times normal alert radius
                rho1 = deg2rad(10);
                delta1 = lambda1*tan(rho1);

                % coordinates of M
                yaw = agents(i).yaw(cumulation-1);
                x_M = agent_x + lambda1*cos(yaw);
                y_M = agent_y + lambda1*sin(yaw);

                reduced_grid = grid_agent_dist < lambda1+rho1;  % not to run through the whole grid

%                 for i:1:world.number_of_tiles_x
%                     for j:1:world.number_of_tiles_y
%                         if euclideanDistance(i,j,agent_x,agent_y) <= lambda1
%                             agents(i).heading(cumulation)
