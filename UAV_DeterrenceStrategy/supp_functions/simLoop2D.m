%% Runs the simulation loop for the 2D strategy
% gives back whether the bird was deterred, deterrence time and length of
% path traveled by the agent

function [deterred valid desired_edge efficient t_deter l_path] = simLoop2D(world, agents, targets, SimParam, q)
...
[b, curve_length] = bezierPath(world, agents(1), targets(1), edge, q, 0);
...
% check the trajectory doesnt enter FID prematurely, exits the function if so
valid = isValidtrajectory(targets(1), FID, b);
if valid == 0
    return
end
...
while t <= t_total && outofBounds == 0
    ...
    for k = 1:size(targets,1)
        ...
        if outofBounds == 1
            ...
            efficient = deterred*valid*desired_edge;
        elseif t == t_total
